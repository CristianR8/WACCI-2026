...
export function PricingCard({ plan, billingPeriod }: Props) {
 ...
 <div
 className={`bg-white rounded-[20px] shadow-one relative z-10 h-full ${
 ...
 {plan.popular && (
 <span className="px-2 py-1 text-xs font-medium rounded-full bg-primary-50 text-primary-500">
 Popular
 </span>
 )}
 </div>
 ...
 ' hover:bg-gray-900 text-white bg-gray-700':
...
function ContactSalesLink({ children }: PropsWithChildren) {
 ...
 <Link
 href="/contact"
 className="block w-full px-8 py-3.5 mt-7 text-sm font-medium text-center rounded-full transition hover:bg-gray-900 text-white bg-gray-700"
 >
 {children}
 </Link>
 );
